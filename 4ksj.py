...
password = 'Hz6wjnN7WXED@ir'  # password，需要修改成自己的密码！！！！！！！！！！！！！！！！！！！！！！！
...
headers = {'Host': 'www.4ksj.vip',  # www.4ksj.com
...
def plusplus(title: str, content: str) -> None:
    if plusplus_token == '':
        ...
        return
    url = 'http://www.pushplus.plus/send?token=' + plusplus_token + '&title=' + quote(title) + '&content=' + quote(
        content)
...
def login(uname, password):
    ...
    time.sleep(1)
    form_hash = re.findall(r'<input type="hidden" name="formhash" value="(.*?)"', ret)[0]
    print('准备登录，获取到登录页formhash: ' + form_hash)
    login_hash = re.findall(r'loginhash=(.*?)"', ret)[0]
    print("准备登陆，获取到登录页loginhash:" + login_hash)
    # 进行登录
    headers['Origin'] = 'https://www.4ksj.com'
    headers['Referer'] = 'https://www.4ksj.com/member.php?mod=logging&action=login'
    headers['Sec-Fetch-Dest'] = 'iframe'
    headers['Content-Type'] = 'application/x-www-form-urlencoded'
    data = {'formhash': form_hash, 'referer': 'https://www.4ksj.com/', 'username': uname, 'password': password,
            'questionid': '0', 'answer': ''}
    ret = r.post(
        'https://www.4ksj.com/member.php?mod=logging&action=login&loginsubmit=yes&loginhash=' + login_hash + '&inajax=1',
        headers=headers, data=urlencode(data)).text
    time.sleep(1)
...
def qiandao():
    ...
    time.sleep(1)
    form_hash = re.findall(r'action=logout&amp;formhash=(.*?)"', ret)[0]
    print('准备签到：获取到签到页formhash: ' + form_hash)
    ...
    ret = r.get('https://www.4ksj.com/qiandao.php?sign=' + form_hash, headers=headers).text
    time.sleep(1)
    if '今天已经打过卡' in ret:
        print('签到结果：今日已签')
...
# 退出登录
def logout(form_hash):
    ret = r.get('https://www.4ksj.com/member.php?mod=logging&action=logout&formhash=' + form_hash, headers=headers).text
    print('退出登录')
